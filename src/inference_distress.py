...
if __name__ == '__main__':
    ...
    utils.setup_logging(log_filename='log_inference_distress')
    logging.info('**** starting distress inference logging ****')
    ...
    y_dev = torch.tensor(dev_modeling_df['distress'], dtype=torch.float32)

    model_nn = torch.load(conf.model_nn_distress_save_path)
    model_nn.eval()
    ...
    outputs_dir = Path(conf.outputs_d2_distress_path).parent
    if not outputs_dir.exists():
        outputs_dir.mkdir()

    dev_pred_df = pd.DataFrame(y_dev_pred)
    dev_pred_df.to_csv(conf.outputs_d2_distress_path, sep='\t', index=False, header=['DevDistressPredictions'])

    logging.info('**** finish distress inference logging ****')
