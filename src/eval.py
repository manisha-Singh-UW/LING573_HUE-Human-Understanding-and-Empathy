...
if __name__ == '__main__':
    ...
    dev_empathy_pred_df = pd.read_csv(conf.outputs_d2_empathy_path, sep='\t')
    y_dev_empathy_pred = dev_empathy_pred_df['DevEmpathyPredictions'].tolist()

    dev_distress_pred_df = pd.read_csv(conf.outputs_d2_distress_path, sep='\t')
    ...
    Path(conf.scores_d2_path).write_text(scores_text)
