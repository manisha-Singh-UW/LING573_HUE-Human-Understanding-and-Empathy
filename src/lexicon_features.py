...
dev_data_path = conf.dev_data_path
...
def create_lex_embedding(dataset_path, emb_model, features, dataset_type, rescale_param_path):
    ...
    emb_df = pd.read_csv(dataset_path + dataset_type + '_' + emb_model, sep = '\t')
    emb_len = len(np.array(ast.literal_eval(emb_df['essay_emb'][0]), dtype=float))
    emb_matrix = np.empty((0,emb_len))
    
    #Get mean and std from embeddings to rescale
    for i in emb_df['essay_emb']:
    ...
    emb_df.drop(columns = 'essay_emb', inplace = True)
    emb_df['essay_emb'] = new_emb_lst
    emb_df.to_csv(dataset_path + dataset_type + '_' + emb_model.split('.')[0] + '_lexicon.tsv',
                  sep = '\t', index = False)


if __name__ == '__main__':

    emb_model = ['data_with_embedding_all-MiniLM-L6-v2.tsv',
            'data_with_embedding_all-mpnet-base-v2.tsv',
            'data_with_embedding_all-roberta-large-v1.tsv',
            'data_with_embedding_text-embedding-ada-002.tsv']
