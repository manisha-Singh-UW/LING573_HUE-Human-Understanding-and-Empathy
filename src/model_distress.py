...
if __name__ == '__main__':
    ...
    utils.setup_logging(log_filename='log_model_distress')
    logging.info('**** starting distress model logging ****')
    ...
    y_val = torch.tensor(val_df['distress'], dtype=torch.float32)
    y_val.unsqueeze_(1)
    ...
    torch.save(model_nn, conf.model_nn_distress_save_path)
    if Path(conf.model_nn_distress_save_path).exists():
        logging.info(f'Model file successfully written to: {conf.model_nn_distress_save_path}')

    logging.info('**** finish distress model logging ****')
