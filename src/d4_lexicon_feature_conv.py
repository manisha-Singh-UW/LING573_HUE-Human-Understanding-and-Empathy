...
train_data_path = conf.train_data_path_conv_d4
dev_data_path = conf.dev_data_path_conv_d4
test_data_path = conf.test_data_path_conv_d4

def lemmatize_essay(dataset):
    lemmatized_essays = []
    lemmatizer = WordNetLemmatizer()

    for essay in dataset['text']:
        lemma_sent = []
...
def create_lex_embedding(dataset_path, emb_model, features, dataset_type, rescale_param_path):
    ...
    for i in tqdm(emb_df['string_embedding']):
    ...
    emb_df.to_csv(dataset_path + 'd4_' + dataset_type + '-' + emb_model.split('.')[0] + '_lexicon_adapt.tsv',
                  sep = '\t', index = False)


if __name__ == '__main__':

    emb_model = ['conv_with_embedding_text-embedding-ada-002.tsv']
